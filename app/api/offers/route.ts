...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    const [ofertas, categorias] = await Promise.all([
      sql`
        SELECT id, nombre, precio_combo, url_imagen, activo, categoria_id
        FROM ofertas
...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  ...
  const empresa_id = EMPRESA_ID;
  const body = (await request.json()) as { precio_combo?: number; activo?: boolean; categoria_id?: string | null };
  ...
  try {
    ...
    if (body.categoria_id !== undefined) {
      await sql`
        UPDATE ofertas SET categoria_id = ${body.categoria_id}
        WHERE id = ${oferta_id} AND empresa_id = ${empresa_id}
      `;
    }
