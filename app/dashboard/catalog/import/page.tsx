...
import type { FilaExcel, ProductoImport, OfertaImport, FragmentoCombo, ResultadoImport } from '@/types';

// ---------------------------------------------------------------------------
// Helpers de parseo
// ---------------------------------------------------------------------------

const REGEX_PREFIJO = /^(PROMO\s+\d+\s+(LLEVA|INCLUYE|X\s+LLEVA|X\s+INCLUYE)\s*:?\s*|COMBO\s*:?\s*|INCLUYE\s*:?\s*)/i;
...
function parsearFragmentos(nombre: string): FragmentoCombo[] {
  return nombre
    ...
    .filter(Boolean)
    .map((parte) => {
      // detectar cantidad al inicio: "3 AGUA 600ML" o "AGUA 600ML x3"
      const matchPre = parte.match(/^(\d+)\s+(.+)/);
      const matchPost = parte.match(/^(.+?)\s+[xX](\d+)$/);
      if (matchPre) {
        return { texto_original: parte, nombre_producto: matchPre[2].trim(), cantidad: parseInt(matchPre[1]), precio_unitario_referencia: 0 };
      } else if (matchPost) {
        return { texto_original: parte, nombre_producto: matchPost[1].trim(), cantidad: parseInt(matchPost[2]), precio_unitario_referencia: 0 };
      }
      return { texto_original: parte, nombre_producto: parte, cantidad: 1, precio_unitario_referencia: 0 };
    });
}

function extraerImagenes(ws: XLSX.WorkSheet): Map<number, { data: string; tipo: string }> {
  const mapa = new Map<number, { data: string; tipo: string }>();
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const images: any[] = (ws as any)['!images'] ?? [];
    for (const img of images) {
      const fila: number = img?.l?.r ?? img?.from?.r ?? -1;
      if (fila >= 0 && img?.data) {
        mapa.set(fila, { data: img.data as string, tipo: (img.type ?? img.ext ?? 'png') as string });
      }
    }
  } catch {
...
function parsearExcel(buffer: ArrayBuffer): { productos: ProductoImport[]; ofertas: OfertaImport[] } {
  // bookImages is a valid runtime option but missing from @types/xlsx — cast needed
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const wb = XLSX.read(buffer, { type: 'array', bookImages: true, dense: true } as any);
  const ws = wb.Sheets[wb.SheetNames[0]];
  const filas = XLSX.utils.sheet_to_json<FilaExcel>(ws);
  const imagenesPorFila = extraerImagenes(ws);
  ...
  filas.forEach((fila, idx) => {
    ...
    const imagen = imagenesPorFila.get(idx + 1); // +1 por header

    if (esCombo(nombreLimpio)) {
      ofertas.push({
        nombre: nombreLimpio.split('+').map((p) => p.trim()).slice(0, 3).join(' + '),
...
export default function ImportarCatalogoPage() {
  ...
  const actualizarFragmento = (ofertaIdx: number, fragIdx: number, campo: keyof FragmentoCombo, valor: string | number) => {
    setOfertas((prev) =>
      prev.map((o, oi) =>
        oi !== ofertaIdx
          ? o
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/dashboard/catalog" className="text-gray-400 hover:text-gray-600 text-sm">← Catálogo</Link>
        ...
        <div className="mt-6 bg-gray-50 rounded-lg p-4">
          ...
          <p className="text-xs text-gray-500 mt-3">
            Filas cuyo NOMBRE contenga <strong>+</strong> se detectan automáticamente como combos/ofertas.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {productos.length} productos · {ofertas.length} combos · {totalImagenes} imágenes detectadas
            </p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setPaso(1)} className="border border-gray-300 text-gray-600 px-4 py-2 rounded-lg text-sm">
              Cambiar archivo
            </button>
            <button
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-gray-600">Fila</th>
                ...
                <th className="px-4 py-3 text-center font-medium text-gray-600">Img</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {productos.map((p) => (
                <tr key={p.fila_numero} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5 text-gray-400 text-xs">{p.fila_numero}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-600">{p.sku}</td>
                  <td className="px-4 py-2.5 text-gray-900 max-w-xs truncate" title={p.nombre}>{p.nombre}</td>
                  <td className="px-4 py-2.5">
                    <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full">Producto</span>
                  </td>
                  <td className="px-4 py-2.5 text-gray-600">{p.categoria}</td>
                  <td className="px-4 py-2.5 text-right text-gray-900">${Number(p.precio).toLocaleString('es-CO')}</td>
                  <td className="px-4 py-2.5 text-right text-gray-600">{p.stock}</td>
                  <td className="px-4 py-2.5 text-center">{p.imagen_base64 ? '🖼️' : '—'}</td>
                </tr>
              ))}
              {ofertas.map((o) => (
                <tr key={o.fila_numero} className="hover:bg-purple-50 bg-purple-50/30">
                  <td className="px-4 py-2.5 text-gray-400 text-xs">{o.fila_numero}</td>
                  <td className="px-4 py-2.5 text-gray-400 text-xs">—</td>
                  <td className="px-4 py-2.5 text-gray-900 max-w-xs truncate" title={o.nombre}>{o.nombre}</td>
                  <td className="px-4 py-2.5">
                    <span className="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full">Combo</span>
                  </td>
                  <td className="px-4 py-2.5 text-gray-400">—</td>
                  <td className="px-4 py-2.5 text-right text-gray-900">${Number(o.precio_combo).toLocaleString('es-CO')}</td>
                  <td className="px-4 py-2.5 text-right text-gray-400">—</td>
                  <td className="px-4 py-2.5 text-center">{o.imagen_base64 ? '🖼️' : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button onClick={() => setPaso(2)} className="border border-gray-300 text-gray-600 px-4 py-2 rounded-lg text-sm">
              ← Volver
            </button>
            <button
              onClick={() => setPaso(4)}
          ...
          {ofertas.map((oferta, oi) => (
            <div key={oi} className="bg-white border border-gray-200 rounded-xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <span className="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full mr-2">Combo</span>
                  <span className="font-semibold text-gray-900">{oferta.nombre}</span>
                </div>
                <span className="text-green-700 font-semibold">${Number(oferta.precio_combo).toLocaleString('es-CO')}</span>
              </div>
              <p className="text-xs text-gray-400 mb-4 italic">&ldquo;{oferta.nombre_original}&rdquo;</p>
                  ...
                  <div key={fi} className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        onChange={(e) => actualizarFragmento(oi, fi, 'nombre_producto', e.target.value)}
                    ...
                    <div className="w-24">
                      ...
                      <input
                        ...
                        min={1}
                        value={frag.cantidad}
                        onChange={(e) => actualizarFragmento(oi, fi, 'cantidad', parseInt(e.target.value) || 1)}
                        className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    <div className="w-28">
                      ...
                      <input
                        ...
                        onChange={(e) => actualizarFragmento(oi, fi, 'precio_unitario_referencia', parseFloat(e.target.value) || 0)}
                        className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                  </div>
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => { setPaso(1); setResultado(null); setProductos([]); setOfertas([]); }}
  ...
  // Paso 4 — antes de importar: resumen de confirmación
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Al confirmar, se ejecutarán las migraciones necesarias, se crearán o actualizarán los
        registros en Neon y se subirán las imágenes a Vercel Blob.
      </p>
